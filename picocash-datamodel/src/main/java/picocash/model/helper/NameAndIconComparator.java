 ...
 * @author wusel
 */
public class NameAndIconComparator implements Comparator<NameAndIconable> {

    private static final Log log = LogFactory.getLog(NameAndIconComparator.class);

    @Override
    public int compare(NameAndIconable one, NameAndIconable two) {
        int erg = one.getName() == null && two.getName() == null ? 0 : one.getName().compareTo(two.getName());
        return erg;
    }
}
